...
jsPsych.plugins['birth-question'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if(trial.questions[i].rows == 1){
        html += '<input style="textAlign:center;" placeholder = "MM" type="text" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns/3+'" value="'+trial.questions[i].value+'"></input>';
        html += '<input style="textAlign:center;" placeholder = "DD" type="text" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns/3+'" value="'+trial.questions[i].value+'"></input>';
        html += '<input style="textAlign:center;" placeholder = "YYYY" type="text" name="#jspsych-survey-text-response-' + i + '" size="'+trial.questions[i].columns/3+'" value="'+trial.questions[i].value+'"></input>';
      } else {
    ...
    display_element.querySelector('#jspsych-survey-text-next').addEventListener('click', function() {
      ...
      for(var index=0; index<matches.length; index++){
        var id = "Q" + index;
        var valMonth = matches[index].childNodes[1].value;
        var valDay = matches[index].childNodes[2].value;
        var valYear = matches[index].childNodes[3].value;
        var obje = {};
        obje[id] = valMonth.toString() + "/" + valDay.toString() + "/" + valYear.toString();
        console.log(Object.assign(question_data, obje));
